...
export default function SearchPage() {
  const [searchOrganizationResults, setOrganizationSearchResults] = useState<
    Organization[]
  >([]);
  ...
  useEffect(() => {
    if (showOrganizations) {
      setOrganizationSearchResults(
        orgSearch.data?.map((org: Organization) => ({
          id: org.id,
          name: org.name,
      ...
      setEventSearchResults(
        eventSearch.data?.map((event: Event) => ({
          id: event.id,
          title: event.title,
      ...
      <div className="mt-12 flex w-full max-w-screen-2xl flex-col px-12">
        ...
        >
          <Button
            ...
            style={{
              marginRight: "10px",
              backgroundColor: showEvents ? "purple" : "white",
              color: showEvents ? "white" : "black",
              outline: "none",
            }}
          >
            Events
          ...
          <Button
            ...
            style={{
              marginRight: "10px",
              backgroundColor: showOrganizations ? "purple" : "white",
              color: showOrganizations ? "white" : "black",
              outline: "none",
            }}
          >
            Organizations
          ...
          <Button
            ...
            style={{
              marginRight: "10px",
              backgroundColor: showUsers ? "purple" : "white",
              color: showUsers ? "white" : "black",
              outline: "none",
            }}
          >
            Users
        ...
        {isLoading ? (
          ...
          searchOrganizationResults.length === 0 &&
          ...
          <div className="grid grid-cols-4 gap-x-4 gap-y-2">
            ...
            {showOrganizations &&
              searchOrganizationResults.map((result, index) => (
                <OrgHostPreview key={index} organization={result} />
              ))}

            {showEvents &&
              searchEventResults.map((result, index) => (
